...
def mock_preprocessor_wizard(preprocessing_config: KVHolder) -> ImagePreprocessorAndChipperWizard:
    ...
    # Create a simple implementation that returns a basic preprocessor
...
def test_atr_inferencer_invalid_image_path(atr_inferencer: ATRInferencer) -> None:
    ...
    with pytest.raises(TypeError):
        atr_inferencer.infer(123)  # Invalid type
    
    with pytest.raises(TypeError):
        atr_inferencer.infer(["/some/path"])  # Invalid type
...
def test_atr_inferencer_boxes_to_polygon(
    ...
    # Get the actual image dimensions from metadata
    num_cols = sicd_metadata.ImageData.NumCols
    num_rows = sicd_metadata.ImageData.NumRows
    
    # Verify clipping works (values should be within image bounds)
...
def test_atr_inferencer_boxes_to_polygon_with_sidd(
    atr_inferencer: ATRInferencer,
    sidd_metadata
) -> None:
    ...
    polygons = atr_inferencer._boxes_to_polygon(bboxes, sidd_metadata)
    
    assert isinstance(polygons, np.ndarray)
    assert polygons.shape == (2, 4, 2)
    
    # Get dimensions from SIDD metadata  
    num_cols = sidd_metadata.Measurement.PixelFootprint.Col
    num_rows = sidd_metadata.Measurement.PixelFootprint.Row
...
def test_atr_inferencer_load_from_config(
    ...
    # Mock the from_preprocessor_config to return our mock wizard
    mock_wizard = mock_preprocessor_wizard(full_inference_config.data_preprocessing)
...
def test_inference_config_kvholder(preprocessing_config: KVHolder) -> None:
    ...
    # Verify preprocessing_config structure from conftest
