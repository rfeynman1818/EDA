...
def compute_metrics(slc_cpu, backend: BackendBase, dr_th=20.0, cfar_th=50, peak_th=25.0) -> Metrics:
    if isinstance(backend, NumpyBackend):
        xp = backend.xp
        ...
        torch = backend.torch
        a = torch.abs(backend.to_backend(slc_cpu))
        i = (a*a).to(torch.float64)
        p1 = torch.quantile(i, 0.01)
        p999 = torch.quantile(i, 0.999)
        dr_db = float(10.0 * torch.log10((p999+EPS)/(p1+EPS)).item())
        med_a = torch.median(a)
        peak_db = float(20.0 * torch.log10((a.max()+EPS)/(med_a+EPS)).item())
        n = i.numel(); topk = max(1, int(0.01*n))
        vals = torch.topk(i.flatten(), topk).values
        nz = float(torch.median(vals).item())
        cfar = cfar_count(a, backend, k=8.0)
        logterm = float(torch.log10(torch.tensor(float(cfar)+1.0, device=a.device)).item())
        score = float(0.55*dr_db + 0.3*peak_db + 0.15*max(0.0, logterm)*20.0)
        bad_flag = (dr_db < dr_th) and (cfar < cfar_th) and (peak_db < peak_th)
        return Metrics(dr_db, peak_db, int(cfar), nz, score, bad_flag)

# --------------------------------- Pipeline ---------------------------------- #

def process_one(path: Path, decimate: int, backend: BackendBase,
                thresholds: Tuple[float,int,float], logger: Optional[logging.Logger]=None) -> dict:
...
def main():
    ...
    if isinstance(backend, TorchBackend) and backend.use_gpu:
        ...
        if workers == 1:
            ...
            # Workers use CPU backend only (safe to import in subprocesses)
